...
/**
 * *** NEW: Sales Summary Tool ***
 * Calculates total sales and monthly breakdown for a party and date range.
 */
async function getSalesSummaryByParty(partyName, startDate, endDate) {
    const cleanPartyName = sanitizeString(partyName).toUpperCase();
    console.log(`Getting sales summary for ${cleanPartyName} between ${startDate} and ${endDate}`);

    try {
        // 1. Get all the bills using the existing function
        const bills = await findBillsByPartyAndDateRange(partyName, startDate, endDate);

        // 2. Check if we got any bills
        if (!bills || bills.length === 0) {
            console.log(`No bills found for ${cleanPartyName} in this period.`);
            return {
        ...
        bills.forEach(bill => {
            ...
            if (!isNaN(amount)) {
                ...
                // Create a key like "2025-04 (Apr)"
                const billDate = new Date(bill.date);
                const monthIndex = billDate.getUTCMonth(); // 0 = Jan, 1 = Feb...
                const year = billDate.getUTCFullYear();
                const monthKey = `${year}-${String(monthIndex + 1).padStart(2, '0')} (${monthNames[monthIndex]})`;
        ...
        // 4. Sort the monthly totals by date
        const sortedMonthlyTotals = Object.keys(monthlyTotals).sort().reduce(
            (obj, key) => { 
        ...
        console.log("Summary calculated:", { totalSales, sortedMonthlyTotals, billCount });
        
        // 5. Return the complete summary object
        return {
            status: "success",
            partyName: cleanPartyName,
        ...
        console.error("Error in getSalesSummaryByParty:", error);
 ...
 */
/**
 * *** NEW: Smart Google Search Tool ***
 * Searches the web and *specifically* looks for Google Maps links.
 */
async function googleSearch(query) {
    ...
    try {
        ...
        if (data.items && data.items.length > 0) {
            ...
            // --- NEW LOGIC: Find the best map link ---
            let foundMapLink = null;
            for (const item of data.items) {
                if (item.link.includes("google.com/maps") || item.link.includes("goo.gl/maps")) {
                    foundMapLink = item.link;
                    break; // Found the best link, stop looking
                }
            }
            // --- END NEW LOGIC ---
            ...
            return { 
                status: "success",
                mapLink: foundMapLink, // Will be the URL or null
                results: results,
                summary: `Found ${data.items.length} results. Top result: ${results[0].snippet}`
            };
        } else {
            return { status: "no_results", mapLink: null, results: [], summary: "No relevant results found on Google." };
        }
    } catch (error) {
