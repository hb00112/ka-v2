...
Your current user's name is ${name}.
Your goal is to be the most professional, all-in-one personal assistant possible.

**--- CORE PHILOSOPHY (CRITICAL) ---**
1.  **BE A "DOER", NOT A "GUIDE":** Your job is to **do the work**.
    * **NEVER** use hesitant phrases ("I'm sorry...", "I can't...").
    * **NEVER** split answers. Give the final, complete answer in **one single message**.
    * **NEVER** tell the user what to do ("you can search...", "you can find...").
2.  **BE DETAILED & IN-DEPTH (NOT CONCISE):**
    * Your replies **MUST** be comprehensive. **NEVER** give one-line answers.
    * **BAD:** "The current Chief Minister of Goa is Pramod Sawant."
    * **GOOD (This is what you MUST do):** "The current Chief Minister of Goa is **Pramod Sawant**, who represents the **Bharatiya Janata Party (BJP)**. He has been in office since **March 19, 2019**, and represents the **Sanquelim** constituency. [Source: Wikipedia](...)."
3.  **USE MARKDOWN FOR CLARITY:**
    * You **MUST** use **bolding** for key terms, names, numbers, and totals to make the response easy to read.

**--- TOOL & DATA RULES (VERY IMPORTANT) ---**
1.  **LOCATION SEARCH (SIMPLE RULE):**
    * When a user asks for *any* location ("Poshak Retail", "CALLICAS CANCONA", etc.), you **MUST** call the \`googleSearch\` tool with the query \`"Google Maps location of [Place Name] Goa"\`.
    * The tool will return a \`mapLink\` and a list of \`results\`.
    * **Scenario 1 (Success):** If \`mapLink\` is **not null**, you **MUST** provide it.
        * **Example:** "Here is the location for **Poshak Retail**: [Google Maps Link](...). The address is **TECHNOPARK, CHOGM ROAD**..."
    * **Scenario 2 (Partial Find):** If \`mapLink\` is **null** but \`results\` exist, you **MUST** report the address/snippet you found and **cite the source** from the \`results\`.
        * **Example:** "I found an address for **Hreenkar Creation**: '**Behind Meat Market, Mapusa**'. A direct map link wasn't found. [Source: Justdial](http://justdial.com/link)."
    * **Scenario 3 (Failure):** If \`status\` is "no_results", just state: "No specific location was found for **[Place Name]**."
2.  **GENERAL SEARCH & NEWS:**
    * For all other web searches, you **MUST** provide a detailed summary (following the "DETAILED & IN-DEPTH" rule) and **cite your sources** with 1-3 clickable Markdown links from the \`results\`.
3.  **FIREBASE & SALES:**
    * Today is ${today}. The current financial year is **${financialYearStart} to ${financialYearEnd}**.
    * For 'total' or 'summary' requests, you **MUST** use the \`getSalesSummaryByParty\` tool.
    * **Example Format:**
        Total Sales for **BARON PANAJI** (FY 2025–26): **₹4,83,270**
        **April:** ₹95,000 | **May:** ₹1,10,000 | **June:** ₹78,270
4.  **LANGUAGE:** Respond in Hindi *only if the user speaks Hindi first*.`;
        }
        ...
        const tools = [
            {
                    ...
                    // --- PASTE THIS NEW TOOL DEFINITION ---
                    {
                      ...
                      {
                        "name": "getSalesSummaryByParty",
                        "description": "Calculates the total sales and a monthly breakdown for a specific party within a date range. Use this for all 'total' or 'summary' requests.",
                        "parameters": {
                            "type": "OBJECT",
                            "properties": {
                                "partyName": {
       ...
       async function processWithAI() {
        ...
        function speakResponse(text) {
            ...
            // 1. Code Blocks (pre) - Must run first
            ...
            // 5. *** NEW: Markdown Links [Title](url) ***
            // This finds [Title](url) and converts it, adding the blue style
            const markdownLinkRegex = /\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g;
            text = text.replace(markdownLinkRegex, '<a href="$2" target="_blank" rel="noopener noreferrer" style="color: #8ab4f8; text-decoration: underline;">$1</a>');

            // 6. *** NEW: Auto-link standalone URLs ***
            // This finds URLs that are NOT part of a markdown link already.
            const urlRegex = /(?<!href=")(?<!\]\()(?<!style="color: #8ab4f8; text-decoration: underline;">)(https?:\/\/[^\s<]+[^\s<.,!?)\]])/g;
            text = text.replace(urlRegex, '<a href="$1" target="_blank" rel="noopener noreferrer" style="color: #8ab4f8; text-decoration: underline;">$1</a>');

            // 7. Newlines (br) - Must run last
            text = text.replace(/\n/g, '<br>');
            
            // 8. Fix newlines inside <pre> tags
        ...
        /**
 * *** NEW: Extracts links from AI text ***
 * Finds all markdown and standalone links, removes them from
 * the text, and returns the clean text + a list of sources.
 */
function extractLinksFromText(text) {
    const sources = [];
    let cleanText = text;

    // 1. Regex for [Title](url)
    const markdownLinkRegex = /\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g;
    cleanText = cleanText.replace(markdownLinkRegex, (match, title, url) => {
        if (!sources.find(s => s.url === url)) { // Avoid duplicates
            sources.push({ title: title, url: url });
        }
        return ''; // Remove from text
    });

    // 2. Regex for standalone http...
    const standaloneLinkRegex = /(?<!href=")(?<!\]\()(?<!style="color: #8ab4f8; text-decoration: underline;">)(https?:\/\/[^\s<]+[^\s<.,!?)\]])/g;
    cleanText = cleanText.replace(standaloneLinkRegex, (match, url) => {
        if (!sources.find(s => s.url === url)) { // Avoid duplicates
            let title = 'Source';
    ...
    // 3. Clean up "Source:" or "Sources:" preambles, newlines, and hanging spaces
    // This removes "Source(s):" if it's left hanging at the end of the text.
    cleanText = cleanText.replace(/(\n|\<br\s*\/?\>)\s*(Sources?:)\s*$/gmi, '');
    cleanText = cleanText.replace(/(\n|\<br\s*\/?\>)\s*$/gmi, ''); // Remove trailing newlines/breaks
    cleanText = cleanText.trim();

    return { cleanText: cleanText, sources: sources };
}

/**
 * *** NEW: Opens the sources modal ***
 */
function openSourcesModal(event, element) {
    event.stopPropagation(); // Stop click from bubbling up
    triggerHapticFeedback();
    
    const list = document.getElementById('sourcesList');
    list.innerHTML = ''; // Clear old sources
    
    let sources = [];
    try {
        // Read the sources from the data attribute
        sources = JSON.parse(element.dataset.sources);
    } catch (e) {
        console.error('Failed to parse sources JSON:', e);
        list.innerHTML = '<p>Error loading sources.</p>';
    }
    
    if (!sources || sources.length === 0) {
        list.innerHTML = '<p>No sources found for this message.</p>';
    } else {
            ...
            const itemHTML = `
                <a href="${source.url}" target="_blank" rel="noopener noreferrer" class="source-item">
                    <div class="source-item-icon">
                        <img src="${faviconUrl}" alt="${hostname}">
                    </div>
                    <div class="source-item-info">
                        <div class="source-item-title" title="${source.title}">${source.title}</div>
                        <div class="source-item-url">${hostname}</div>
                    </div>
                </a>
            `;
    ...
    document.getElementById('sourcesOverlay').classList.add('active');
    document.getElementById('sourcesModal').classList.add('active');
}

/**
 * *** NEW: Closes the sources modal ***
 */
function closeSourcesModal() {
    document.getElementById('sourcesOverlay').classList.remove('active');
    document.getElementById('sourcesModal').classList.remove('active');
}

// ... (before your copyCode function) ...
        ...
        function scrollToBottom(instant = false) { const area = document.getElementById('conversationArea'); area.scrollTo({ top: area.scrollHeight, behavior: instant ? 'auto' : 'smooth' }); }
            ...
            if (type === 'typing') { 
                ...
                let sources = [];
                let textForFormatting = text;
                
                if (type === 'assistant' && text) {
                    const extracted = extractLinksFromText(text);
                    textForFormatting = extracted.cleanText;
                    sources = extracted.sources;
                }
                
                const formattedText = textForFormatting ? parseMarkdown(textForFormatting) : '';
                ...
                if (file) { 
                    if (file.mimeType.startsWith('image/')) { 
                        attachmentHTML = `<div class="message-attachment"><img src="${file.dataUrl}" alt="${file.name}"></div>`; 
                    } else { 
                        attachmentHTML = `<div class="message-attachment"><div class="message-attachment-file">${fileIconSVG}<span>${file.name}</span></div></div>`; 
                    } 
                }
                ...
                if (type !== 'system') {
                    ...
                    if (type === 'assistant' && sources.length > 0) {
                        const favicons = sources.map(s => {
                            let hostname = 'source.com';
                            try { hostname = new URL(s.url).hostname; } catch(e) {}
                            // *** CHANGE 1: Request sz=64 for a sharper icon ***
                            return `<img src="https://www.google.com/s2/favicons?domain=${hostname}&sz=64" class="source-icon" alt="${hostname}" title="${hostname}">`;
                        }).slice(0, 3).join('');
                        
                        const sourcesJSON = JSON.stringify(sources).replace(/'/g, '&quot;');
                        
                        sourcesPillHTML = `
                            <div class="sources-pill" onclick='openSourcesModal(event, this)' data-sources='${sourcesJSON}'>
                                <div class="source-icon-stack">
                                    ${favicons}
                                </div>
                                <span>Sources</span>
                            </div>
                    ...
                    // *** CHANGE 2: Re-ordered HTML. Moved ${sourcesPillHTML} to be after message-actions ***
                    bubble.innerHTML = `
                        <div class="message-label">${type === 'user' ? 'You' : 'ka vent'}</div>
                        ${attachmentHTML}
                        ${textForFormatting ? `<div class="message-content" data-text="${textForFormatting.replace(/"/g, '&quot;')}">${formattedText}</div>` : ''}
                        ${(textForFormatting || attachmentHTML) ? `<div class="message-actions">${actionsHTML}</div>` : ''}
                        ${sourcesPillHTML} 
                    `;
                } else { 
    ...
    "googleSearch": googleSearch,
        "addBill": addBill,
         "getSalesSummaryByParty": getSalesSummaryByParty, // <-- **** ADD THIS LINE ****
        "readList": readList,
        "findData": findData,
